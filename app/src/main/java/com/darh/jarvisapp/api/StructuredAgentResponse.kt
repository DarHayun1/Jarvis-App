...
data class StructuredAgentResponse(
    ...
    companion object {
        ...
        private fun jsonSchema(): String {
            return "{\n" +
                    " \"$THOUGHT_FIELD\": \"string (evaluation of the task and a plan to solve it)\"\n" +
                    " \"$NEXT_TOOL_FIELD\": string? (The next tool to be used in order to complete our plan. The tool must be on of the provided \"action_tools\" and follow the format.)\n"+
                    "}"
        }
        ...
        "You job is to complete tasks or answer questions by chaining actions using the \"tools\" that will be provided next.\n"
...
//                    "\""
        ...
        fun structureSetup(tools: List<AgentTool>) =
            "Your next response must be in the 'BASE' JSON schema that'll be provided next.\n" +
                    ...
                    "For example for a user saying he wants to eat good sushi tonight in Tel aviv but wants to save money:\n" +
                    "{\n" +
